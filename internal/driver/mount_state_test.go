...
func TestPersistedMNMWMountArgs(t *testing.T) {
	canonical := func() []string {
		return []string{
			"mount", "--foreground", "--server", "https://api.drive9.ai",
			"--profile", profileNone,
			"--durability", durabilityCloseSync,
			"--attr-ttl", "30s",
			":/k8s/pvc/volume", "/var/lib/kubelet/plugins/kubernetes.io/csi/pv/volume/globalmount",
		}
	}
	withoutOption := func(args []string, option string) []string {
	...
	tests := []struct {
		...
		{name: "canonical", mutate: func(args []string) []string { return args }, wantOK: true},
		{name: "unrelated options differ", mutate: func(args []string) []string {
			args[3] = "https://another.drive9.example"
			args[9] = "60s"
			return append(args[:len(args)-2], "--entry-ttl", "45s", "--perf-dir", "/perf", args[len(args)-2], args[len(args)-1])
		}, wantOK: true},
		{name: "missing profile", mutate: func(args []string) []string {
			return withoutOption(args, "--profile")
		}},
		{name: "missing durability", mutate: func(args []string) []string {
			return withoutOption(args, "--durability")
		}},
		{name: "duplicate profile", mutate: func(args []string) []string {
			return append(args[:len(args)-2], "--profile", profileNone, args[len(args)-2], args[len(args)-1])
		}},
		{name: "duplicate durability", mutate: func(args []string) []string {
			return append(args[:len(args)-2], "--durability", durabilityCloseSync, args[len(args)-2], args[len(args)-1])
		}},
		{name: "different profile", mutate: func(args []string) []string {
			args[5] = "coding-agent"
			return args
		}},
		{name: "different durability", mutate: func(args []string) []string {
		...
		{name: "additional profile equals alias", mutate: func(args []string) []string {
			return append(args[:len(args)-2], "--profile=none", args[len(args)-2], args[len(args)-1])
		}},
		{name: "durability equals alias", mutate: func(args []string) []string {
			args = withoutOption(args, "--durability")
			return append(args[:len(args)-2], "--durability=close-sync", args[len(args)-2], args[len(args)-1])
		}},
		{name: "additional durability equals alias", mutate: func(args []string) []string {
			return append(args[:len(args)-2], "--durability=close-sync", args[len(args)-2], args[len(args)-1])
		}},
		{name: "single-dash profile override", mutate: func(args []string) []string {
			return append(args[:len(args)-2], "-profile", "coding-agent", args[len(args)-2], args[len(args)-1])
		}},
		{name: "single-dash durability override", mutate: func(args []string) []string {
			return append(args[:len(args)-2], "-durability=auto", args[len(args)-2], args[len(args)-1])
		}},
		{name: "malformed profile pair", mutate: func(args []string) []string {
			args = withoutOption(args, "--profile")
			return append(args[:len(args)-2], "--profile", args[len(args)-2], args[len(args)-1])
		}},
		{name: "malformed durability pair", mutate: func(args []string) []string {
			args = withoutOption(args, "--durability")
			return append(args[:len(args)-2], "--durability", args[len(args)-2], args[len(args)-1])
		}},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			state := mountState{MountArgs: test.mutate(canonical())}
			err := validatePersistedMNMWMountArgs(state, profileNone, durabilityCloseSync)
			if test.wantOK && err != nil {
				t.Fatalf("validatePersistedMNMWMountArgs() error = %v", err)
			}
			if !test.wantOK && err == nil {
				t.Fatal("validatePersistedMNMWMountArgs() accepted invalid primary argv")
			}
		})
	}
}

func TestPersistedMNMWMountArgsAcceptsWriteSync(t *testing.T) {
	state := mountState{MountArgs: []string{
		"mount", "--foreground", "--profile", profileNone,
		"--durability", durabilityWriteSync, ":/k8s/pvc/volume", "/staging",
	}}
	if err := validatePersistedMNMWMountArgs(state, profileNone, durabilityWriteSync); err != nil {
		t.Fatalf("validatePersistedMNMWMountArgs() error = %v", err)
	}
}

func TestPersistedMNMWMountArgsValidatesFallback(t *testing.T) {
	primary := []string{
		"mount", "--foreground", "--profile", profileNone,
		"--durability", durabilityCloseSync,
		":/k8s/pvc/volume", "/var/lib/kubelet/plugins/kubernetes.io/csi/pv/volume/globalmount",
	...
	for _, test := range []struct {
		...
		{name: "canonical", fallback: append([]string(nil), primary...), wantOK: true},
		{name: "missing profile", fallback: []string{
			"mount", "--foreground", "--durability", durabilityCloseSync,
			":/k8s/pvc/volume", "/var/lib/kubelet/plugins/kubernetes.io/csi/pv/volume/globalmount",
		}},
		{name: "different durability", fallback: []string{
			"mount", "--foreground", "--profile", profileNone,
			"--durability", durabilityWriteSync,
			":/k8s/pvc/volume", "/var/lib/kubelet/plugins/kubernetes.io/csi/pv/volume/globalmount",
		}},
		{name: "alias", fallback: []string{
			"mount", "--foreground", "--profile", profileNone,
		...
		t.Run(test.name, func(t *testing.T) {
			state := mountState{
				MountArgs:         append([]string(nil), primary...),
				FallbackMountArgs: test.fallback,
			}
			err := validatePersistedMNMWMountArgs(state, profileNone, durabilityCloseSync)
			if test.wantOK && err != nil {
				t.Fatalf("validatePersistedMNMWMountArgs() error = %v", err)
			...
			if !test.wantOK && err == nil {
				t.Fatal("validatePersistedMNMWMountArgs() accepted invalid fallback argv")
			}
		})
	}
}
