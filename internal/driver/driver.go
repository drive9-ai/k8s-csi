...
func (d *Driver) validatePublishedMount(volumeID string, stagingTarget string, target string, readonly bool, accessMode string) error {
	...
	if state.Layout == publishLayoutSubtree {
		...
		if err != nil {
			return status.Errorf(codes.Internal, "compare workspace target mount: %v", err)
		}
		if !same {
			return status.Error(codes.FailedPrecondition, "workspace target is mounted for a different Drive9 volume")
		}
	}
	return nil
}
...
func cleanupPublishTargetWithOps(state publishState, isMounted func(string) (bool, error), unmountAll func(string) error, removePath func(string) error) error {
	...
	if state.Layout == publishLayoutSubtree {
		...
		if err != nil {
			return fmt.Errorf("check mount %s: %w", workspaceTarget, err)
		}
		if mounted {
